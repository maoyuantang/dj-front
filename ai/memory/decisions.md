...
## 2026-07-19：微信小程序首发与多端平台适配边界

- 背景：商城首发微信小程序，未来考虑 H5/App；登录、支付、分享和权限的平台 API 不同。
- 决定：页面和业务 Store 只依赖稳定的领域 API；微信登录、支付、分享和权限能力通过 `src/platform/` 适配器提供。短信登录作为跨端通用能力，微信手机号授权仅在微信小程序编译目标出现。
- 原因：保持商城页面与平台 SDK 解耦，并避免无关平台实现进入当前包。
- 影响：新增 H5/App 时实现对应适配器和平台配置；商品、购物车、订单等页面及后端业务接口保持复用。适配器不能用静默 mock 代替缺失配置。
- 状态：已接受并开始落地。
